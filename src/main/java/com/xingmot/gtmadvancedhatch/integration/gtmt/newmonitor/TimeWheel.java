...
public class TimeWheel {
    ...
    public BigDecimal getAvgByTick() {
        if (lastUpdateTick - firstUpdateTick < slotResolution * slotNum)
            return new BigDecimal(sum).divide(BigDecimal.valueOf(lastUpdateTick - firstUpdateTick + 1), RoundingMode.HALF_UP);
        return slots.isEmpty() ?
                BigDecimal.ZERO :
                new BigDecimal(sum)
                        .divide(BigDecimal.valueOf((long) slots.size() * slotResolution + lastUpdateTick % slotResolution - slotResolution), RoundingMode.HALF_UP);
    }
}
